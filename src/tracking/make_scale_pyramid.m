function [pyramid, pad_masks_x] = make_scale_pyramid(im, targetPosition, in_side_scaled, out_side, avgChans, stats, p)
    ...
    if p.merge_factor > 0
        [search_region, ~, pad_mask_region] = get_subwindow_mask(im, targetPosition, [search_side search_side], [max_target_side max_target_side], avgChans, p.gpus, []);
    else
    ...
    for s = 1:n
        ...
        if p.merge_factor > 0            
            [pyramid(:,:,:,s), ~, pad_masks_x(:,:,:,s)] = get_subwindow_mask(search_region, (1+search_side*[1 1])/2, [out_side out_side], target_side*[1 1], avgChans, p.gpus, pad_mask_region);
        else
